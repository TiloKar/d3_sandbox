...
*/
  ...
  onprogressLoadingXHRFile(xhr){
    ...
    if (local===true){
      var checkCRCinFile = function () {
        if ( (trendfile1.lineCRCerrors > 0)||
             (trendfile1.lineFrameError > 0)||
             (trendfile1.eventCRCerrors > 0)||
             (trendfile1.eventFrameError > 0)){
      ...
      //alert(trendfile1.eventFrameError);
    }else{
      var checkCRCinFile = function () {
        if ( (trendfile1.lineCRCerrors > 0)||
             (trendfile1.lineFrameError > 0)||
    ...
    if  (!trendfile1.firstLineOK) {//vorprüfung nach constructor
      $( "#DialogErrorHeaderTrendfile" ).dialog("open");
      //alert("pre");
    }else {
			$("#editor").hide();
  ...
  */
  unzipDumpFile1(response,local){
      ...
      //hier kein asynchrones entpacken  !!!
      if  (false)  //resreviert für zukünftige auswertu der checksummen beim dumpfile Entpacken
      //müsste als methode in BinaryBRTypedFile angelegt sein, also am besten vererben und dann auf die spezifischen crc elemente prüfen, solan gder blob noch im scope ist
      {
        $( "#DialogErrorHeaderTrendfile" ).dialog("open");
      }else {
        if (local===true){
  ...
  unzipEventFile(response,local){
    ...
    var checkCRCinFile = function () {
      if ( (!eventfile1.crcHeader_OK > 0)||
           (eventfile1.eventCRCerrors > 0)||
           (eventfile1.eventFrameError > 0)){
        $( "#DialogErrorHeaderTrendfile" ).dialog("open");
      }else{
        HMI.onFinishUnzip();
      }
